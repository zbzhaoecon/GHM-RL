"""
Training script for Monte Carlo Policy Gradient on GHM Model with Time-Augmented State.

This script trains the MonteCarloPolicyGradient solver on the time-augmented GHM
equity dynamics where the state is (c, τ) with τ = time-to-horizon.

This correctly handles finite-horizon problems where the optimal policy depends on
remaining time, avoiding the issue where early and late-episode behaviors get mixed up.

Usage:
    # Start new training
    python scripts/train_monte_carlo_ghm_time_augmented.py
    python scripts/train_monte_carlo_ghm_time_augmented.py --n_iterations 10000 --lr_policy 3e-4
    python scripts/train_monte_carlo_ghm_time_augmented.py --seed 456 --device cuda

    # Resume from checkpoint
    python scripts/train_monte_carlo_ghm_time_augmented.py --resume checkpoints/monte_carlo_time_aug/step_5000.pt
"""
...
class TrainConfig:
    ...
    eval_freq: int = 1000
    ckpt_freq: int = 5000
    ckpt_dir: str = "checkpoints/monte_carlo_time_aug"
...
def compute_policy_value_for_visualization(
    ...
    For time-augmented dynamics, computes on 2D grid (c, τ).
    For standard dynamics, computes on 1D grid (c).
    ...
    # Check if time-augmented (2D state)
    if state_space.dim == 2:
        ...
        # Also compute slices at specific tau values for line plots
        tau_slices = [0.5, state_space.upper[1].item() * 0.25,
                      state_space.upper[1].item() * 0.5,
                      state_space.upper[1].item() * 0.75,
                      state_space.upper[1].item()]
        results['tau_slices'] = tau_slices
        results['dividend_slices'] = []
        results['equity_slices'] = []
        results['value_slices'] = []

        for tau_val in tau_slices:
            c_tensor = torch.tensor(c_values, dtype=torch.float32).unsqueeze(1)
            tau_tensor = torch.full((n_points, 1), tau_val, dtype=torch.float32)
            states_slice = torch.cat([c_tensor, tau_tensor], dim=1).to(device)

            with torch.no_grad():
                actions_slice, _ = policy.sample(states_slice, deterministic=True)
                results['dividend_slices'].append(actions_slice[:, 0].cpu().numpy())
                results['equity_slices'].append(actions_slice[:, 1].cpu().numpy())

                if baseline is not None:
                    values_slice = baseline(states_slice).squeeze().cpu().numpy()
                    results['value_slices'].append(values_slice)
                else:
                    results['value_slices'].append(np.zeros(n_points))

    else:
        # 1D case (original code)
        c_values = np.linspace(state_space.lower[0].item(), state_space.upper[0].item(), n_points)
...
def main():
    ...
    # 1. Setup dynamics
    # =========================================================================
    print("\n[1/6] Setting up GHM Equity time-augmented dynamics...")
    params = GHMEquityParams()
    dynamics = GHMEquityTimeAugmentedDynamics(params, T=config.T)
    print(f"  State dimension: {dynamics.state_space.dim} (c, τ)")
